...
Commands:
  ...
  erd from-link   공유 링크에서 layout.json / memos.json 복원
  init            대화형 셋업 안내
...
편집 모드에서 만든 배치를 `layout.json` / `memos.json` 으로 되돌린다.
...
- `positions` / `colors` / `memos` 가 하나도 없으면 아무것도 쓰지 않고 에러로 끝난다.
...
- `layout.json` / `memos.json` 은 **선택**이다. 없으면 자동 배치 + 메모 없음으로 동작한다.
...
|---|---|
| `⌘K` / `Ctrl+K` | 커맨드 팔레트 |
| `⌘C` / `Ctrl+C` | 현재 링크 복사 (텍스트를 선택한 상태면 일반 복사로 동작) |
| `⇧1` | 화면에 맞추기 |
| `⇧2` | 모든 컬럼 표시 |
| `⇧3` | 테이블 이름만 |
| `⇧4` | 키만 |
...
> `⌘C` 는 현재 URL 을 클립보드에 넣는다. 배치·색상·메모가 전부 URL 에 실려 있으므로
> **이 한 번의 복사가 곧 공유**다.
...
| 테이블 | 색상 팔레트 |
| 메모 | 색상 팔레트 + 폰트 크기(`−` / 숫자 입력 / `+`) + `Delete memo` |

### 메모

- 편집 모드에서 메모 본문은 textarea 가 되어 바로 타이핑할 수 있다.
- 우하단 핸들을 끌어 크기를 조절한다. 최소 `100 × 60`, 기본 `220 × 120`.
- 폰트 크기는 `10`~`28`, 기본 `13`, 버튼은 2씩 증감한다.
...
```bash
# 1. ?edit=1 로 열어 배치·색상·메모 정리
# 2. ⌘C 로 링크 복사
# 3. 링크를 파일로 되돌린다
npx erdkit erd from-link --input '<복사한 URL>' --output-dir dist
# 4. dist/layout.json, dist/memos.json 을 소스에 커밋
```

**B. Export 메뉴에서 다운로드**

편집 모드에서 `Export` → `Download layout.json` / `Download memos.json`.
...
| `fontSize` | number | | `13` | `10`~`28` 로 잘린다 |
...
`positions` / `colors` / `memos` 는 **deflate 압축 + URL-safe base64**(`+`→`-`,
...
- `memos` 만 목록이 아니라 단일 JSON blob 이다. 메모 본문은 자유 형식이라
  목록 파서의 `split(',')` 에 잘려나가기 때문.

### 히스토리 동작

`push` 는 뒤로가기 스택에 쌓이고 `replace` 는 쌓이지 않는다.
탐색(`active`/`show`/`hidden`)은 뒤로가기가 동작해야 하고, 편집(`positions`/`colors`/`memos`)은
드래그 한 번마다 히스토리를 채우면 안 되기 때문에 의도적으로 나눴다.
...
`layout.json` / `memos.json` 은 뷰어가 `cache: 'no-cache'` 로 요청해 브라우저 캐시를
재검증한다. 하지만 **CDN 캐시는 별개**다. 배포 후 즉시 반영되게 하려면 CDN 쪽에서
이 두 파일의 TTL 을 짧게 잡거나 무효화(invalidation)를 걸어야 한다.
...
**`from-link` 가 "carries no positions, colors or memos" 로 실패한다**
...
**배포했는데 예전 배치가 보인다**
CDN 캐시다. `layout.json` / `memos.json` 을 무효화한다.

**초기화하고 싶다**
브라우저 콘솔에서 `liamLayout.reset()` / `liamMemos.reset()`.
이 브라우저의 편집 내역만 지우고 `layout.json` / `memos.json` 상태로 돌아간다.
...
원본에는 위치 영속화, 메모, 색상, 편집 모드, MySQL export 가 없다.
