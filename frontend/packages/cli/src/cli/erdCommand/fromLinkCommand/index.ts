...
export const parseLink = (url: string): LinkContents => {
  ...
  return { layout, memos: parsedMemos }
}
...
export const fromLinkCommand = async (
  ...
  const { layout, memos } = contents
  if (layout === null && memos === null) {
    return [
      new ArgumentError(
        'The link carries no `positions`, `colors` or `memos`. Open the ERD with `?edit=1`, arrange it, then copy the URL.',
      ),
    ]
  }
